...
def display_camera():
    ...
    # Draw text with probability on the frame
    ...
    for (x, y, w, h) in faces:
        if predicted_class[0] == 0:
            text = f"Mask: {mask_probability:.2f}%"
            color = (0, 255, 0)
        else:
            text = f"No Mask: {100 - mask_probability:.2f}%"
            color = (0, 0, 255)
            ...
            status_label.config(text=f"Captured image of someone without a mask")
